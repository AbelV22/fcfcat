/**
 * scrape-team — Supabase Edge Function (Deno)
 *
 * POST { slug, competition, group, season, team_name }
 *   → Scrapes all actas for the team from FCF, writes fcf_player_stats
 *     and fcf_referee_matches, updates scrape_jobs row.
 */
...
Deno.serve(async (req: Request) => {
  ...
  let body: { slug: string; competition: string; group: string; season?: string; team_name?: string }
  try {
    body = await req.json()
  } catch {
  ...
  if (!slug || !competition || !group) {
    return jsonResp({ error: 'Missing required fields: slug, competition, group' }, 400)
  }

  const jobId = `${season}-${competition}-${slug}`

  // ── Upsert job row — status=running ──────────────────────────────────────
  ...
  // Run scraping asynchronously — returns HTTP response immediately
  EdgeRuntime.waitUntil(runScrape({
    SUPABASE_URL,
    SERVICE_KEY,
    jobId,
    slug,
    teamName: team_name || slug,
    competition,
    group,
    season,
  }))

  return jsonResp({ jobId, status: 'running' })
})
...
async function runScrape(opts: {
  ...
  try {
    ...
    const actaUrls = await fetchActaUrls(SUPABASE_URL, SERVICE_KEY, slug, competition, group, season)

    await dbPatch(SUPABASE_URL, SERVICE_KEY, 'scrape_jobs', jobId, {
      actas_found: actaUrls.length,
      status: 'running',
    })
    ...
    for (let i = 0; i < statsToUpsert.length; i += 50) {
      await dbUpsert(SUPABASE_URL, SERVICE_KEY, 'fcf_player_stats', statsToUpsert.slice(i, i + 50))
    }
    ...
    for (let i = 0; i < refRows.length; i += 50) {
      await dbUpsert(SUPABASE_URL, SERVICE_KEY, 'fcf_referee_matches', refRows.slice(i, i + 50))
    }
...
function accumulatePlayerStats(
  ...
  // Determine which side matches our team slug
  // Try exact match first, then prefix/contains matching
  ...
  if (!mySide) {
    // Can't determine team side — skip player stats but still store match data
    console.warn(`[accumulatePlayerStats] could not match slug ${teamSlug} to home=${homeActaSlug} or away=${awayActaSlug}`)
    return
  }
  ...
  // Build a lookup of who came in as sub at what minute
  const subInMinute: Record<string, number> = {}
  for (const sub of mySubs) {
    subInMinute[slugify(sub.playerIn)] = sub.minute
    // Player who went out only played until sub.minute — handled via minutes_played
  }

  // Build lookup of who came out (to calculate minutes for starters who were subbed off)
  const subOutMinute: Record<string, number> = {}
  for (const sub of mySubs) {
    subOutMinute[slugify(sub.playerOut)] = sub.minute
  }

  // Red card minutes for minute adjustment
  ...
  for (const p of myPlayers) {
    ...
    if (p.isStarter) {
      ps.appearances++
      ps.starts++
      // Minutes: 90 minus time subbed out (if applicable), minus time after red card
      let mins = 90
      ...
      // Bench player — only counts if they came in
  ...
  // Yellow cards for our team's players
  ...
  // Red cards for our team's players
  for (const card of myReds) {
    if (card.is_double_yellow_dismissal) {
      // Double yellow = 1 extra yellow + 1 red
      getOrCreate(map, card.player).yellow_cards++
...
async function fetchActaUrls(
  ...
  const [homeRes, awayRes] = await Promise.all([
    fetch(`${base}?${common}&home_slug=eq.${enc(slug)}`, { headers: sbHeaders(SERVICE_KEY) }),
    fetch(`${base}?${common}&away_slug=eq.${enc(slug)}`, { headers: sbHeaders(SERVICE_KEY) }),
  ])
...
async function scrapeActa(url: string): Promise<ActaData | null> {
  ...
  if (!jornada) {
    console.warn(`[scrapeActa] no jornada found in ${url}`)
    return null
  }
  ...
  // Split HTML at "Comparativa" to separate home vs away sections
  ...
  const cards = extractCards(html, homePart, awayPart)
...
function extractMatchInfo(html: string): {
  ...
  // Primary: acta-table-header with 3 TDs: home | score | away
  ...
  // Fallback: acta-equip + acta-resultat
...
function extractCards(html: string, homePart: string, awayPart: string): CardEntry[] {
