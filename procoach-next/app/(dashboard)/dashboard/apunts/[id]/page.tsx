    ...
    <div className="min-h-screen bg-[#08090a]">
      ...
      <header className="border-b border-white/[0.06] bg-[#0a1628]/80 backdrop-blur sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <span className="text-sm font-medium text-white">vs {note.opponent}</span>
          <Link
            href={`/dashboard/apunts/nou?edit=${note.id}`}
            className="flex items-center gap-1.5 text-xs text-[#62666d] hover:text-white transition-colors"
          >
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="text-center">
          <p className={`text-5xl font-medium ${resultColor}`}>
            {note.goals_for ?? '?'} — {note.goals_against ?? '?'}
          </p>
          <p className="text-lg font-medium text-white mt-2">vs {note.opponent}</p>
          <div className="flex items-center justify-center gap-4 mt-2 text-xs text-[#62666d]">
          ...
          {note.tactical_approach && (
            <span className="inline-block mt-2 px-3 py-1 bg-white/5 rounded-full text-xs text-[#8a8f98]">
        ...
        {starters.length > 0 && (
          <section className="v2-card p-5">
            <h3 className="text-xs font-medium text-[#62666d] uppercase tracking-wider mb-3 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {starters.map((l) => (
                <div key={l.player_name} className="flex items-center gap-2 py-1.5 px-2 bg-white/3 rounded-lg">
                  <span className="text-[9px] font-medium text-green-400 w-6">{l.role || '—'}</span>
            ...
            {bench.length > 0 && (
              <>
                <p className="text-[10px] text-[#62666d] mt-3 mb-1.5">Banqueta</p>
                <div className="flex flex-wrap gap-1.5">
                  {bench.map((l) => (
                    <span key={l.player_name} className="text-[11px] px-2 py-0.5 bg-white/3 text-[#8a8f98] rounded">
                      {l.player_name}
                    </span>
                  ))}
                </div>
            ...
            <div className="space-y-2">
              {note.events.map((e) => (
                <div key={e.id} className="flex items-center gap-3 py-1.5">
                  <span className="text-xs font-mono text-[#62666d] w-8 text-right">{e.minute}&apos;</span>
                  <div className="flex-1">
                    <span className="text-xs font-medium text-white">{e.player_name}</span>
                    ...
                    {e.note && <p className="text-[10px] text-[#62666d] mt-0.5">{e.note}</p>}
                  </div>
                </div>
              ))}
            </div>
                ...
                return (
                  <div key={r.id} className="flex items-start gap-3 py-2 px-3 bg-white/3 rounded-lg">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-white">{r.player_name}</p>
                      {r.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {r.tags.map((t) => (
                            <span key={t} className="text-[9px] px-1.5 py-0.5 bg-white/5 text-[#62666d] rounded">
                              {tagMap[t] || t}
                            </span>
                          ))}
                        </div>
                      )}
                      {r.note && <p className="text-[10px] text-[#62666d] mt-1">{r.note}</p>}
                    </div>
                  </div>
                )
              })}
        ...
        {(note.key_moments || note.opponent_analysis || note.areas_to_improve) && (
          <section className="v2-card p-5 space-y-4">
            <h3 className="text-xs font-medium text-[#62666d] uppercase tracking-wider flex items-center gap-2">
              <MessageSquare size={12} /> Notes tàctiques
            </h3>
            {note.key_moments && (
              <div>
                <p className="text-[10px] text-[#62666d] mb-1">Moments clau</p>
                <p className="text-xs text-[#d0d6e0] whitespace-pre-wrap">{note.key_moments}</p>
              </div>
            )}
            {note.opponent_analysis && (
              <div>
                <p className="text-[10px] text-[#62666d] mb-1">Anàlisi del rival</p>
                <p className="text-xs text-[#d0d6e0] whitespace-pre-wrap">{note.opponent_analysis}</p>
              </div>
            )}
            {note.areas_to_improve && (
              <div>
                <p className="text-[10px] text-[#62666d] mb-1">Àrees de millora</p>
                <p className="text-xs text-[#d0d6e0] whitespace-pre-wrap">{note.areas_to_improve}</p>
              </div>
            )}
          </section>
        )}
