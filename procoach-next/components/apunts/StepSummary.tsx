...
function StatStepper({ value, onChange, label }: { value: number | null; onChange: (v: number | null) => void; label?: string }) {
  ...
  return (
    <div className="flex items-center gap-1">
      {label && <span className="text-[9px] text-[#8a8f98] w-16 shrink-0">{label}</span>}
      <button
        onClick={() => onChange(Math.max(0, v - 1))}
        className="w-7 h-7 rounded-lg bg-white/5 border border-white/10 text-[#8a8f98] hover:text-white hover:bg-white/10 flex items-center justify-center transition-all active:scale-90"
      >
        <Minus size={12} />
      </button>
...
function ForAgainstStepper({
  ...
  return (
    <div className="flex items-center justify-between py-2">
      <span className="text-xs text-[#d0d6e0] flex-1">{label}</span>
      <div className="flex items-center gap-3">
        <StatStepper value={forValue} onChange={onForChange} />
        <span className="text-[10px] text-[#62666d]">vs</span>
  ...
  return (
    <div className="flex items-center justify-between py-1.5">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[8px] text-[#22c55e]/50 w-7 text-right">auto</span>
      </div>
    </div>
  )
}
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-2 mb-5">
        <div className="glass-card rounded-xl p-3 text-center">
          ...
          <p className="text-[10px] text-[#8a8f98]">Resultat</p>
        </div>
        <div className="glass-card rounded-xl p-3 text-center">
          <Clock size={14} className="text-amber-400 mx-auto mb-1" />
          <p className="text-sm font-black text-white">
            {auto.goals_1h_for}-{auto.goals_1h_against}
            <span className="text-[9px] text-[#8a8f98] font-normal"> / </span>
            {auto.goals_2h_for}-{auto.goals_2h_against}
          </p>
          <p className="text-[10px] text-[#8a8f98]">1a / 2a part</p>
        </div>
        <div className="glass-card rounded-xl p-3 text-center">
          ...
          <p className="text-[10px] text-[#8a8f98]">Tirs totals</p>
        </div>
        <div className="glass-card rounded-xl p-3 text-center">
          <TrendingUp size={14} className="text-[#22c55e] mx-auto mb-1" />
          <p className="text-lg font-black text-white">{totalEvents}</p>
          <p className="text-[10px] text-[#8a8f98]">Events</p>
        </div>
      </div>

      {/* ─── Half-time score ─── */}
      <div className="mb-5">
        <label className="text-xs text-[#8a8f98] mb-2 block">Resultat descans</label>
        <div className="flex items-center gap-2">
          <input
      ...
      <StatSection title="Atac" icon={Crosshair} iconColor="#4ade80" defaultOpen>
        <div className="flex items-center justify-between mb-2">
          <span className="text-[9px] text-[#62666d] uppercase tracking-wider">Nosaltres</span>
          <span className="text-[9px] text-[#62666d] uppercase tracking-wider">Rival</span>
        </div>
        ...
        <div className="border-t border-white/5 pt-2 mt-2">
          <p className="text-[9px] text-[#62666d] uppercase tracking-wider mb-2">Manual</p>
          ...
          <div className="flex items-center justify-between py-2">
            <span className="text-xs text-[#d0d6e0] flex-1">Regates</span>
            <div className="flex items-center gap-2">
      ...
      {/* DISTRIBUCIO */}
      <StatSection title="Distribucio" icon={Waypoints} iconColor="#22c55e">
        ...
        <div className="border-t border-white/5 pt-2 mt-2">
          ...
          <div className="flex items-center justify-between py-2">
            <span className="text-xs text-[#d0d6e0] flex-1">Precisio passada</span>
            <div className="flex items-center gap-2">
              <input
      ...
      <StatSection title="Defensa" icon={Shield} iconColor="#f59e0b">
        ...
        <div className="border-t border-white/5 pt-2 mt-2">
          ...
          <div className="flex items-center justify-between py-2">
            <span className="text-xs text-[#d0d6e0] flex-1">Despejos</span>
            <StatStepper value={summary.clearances} onChange={(v) => update({ clearances: v })} />
          </div>
          <div className="flex items-center justify-between py-2">
            <span className="text-xs text-[#d0d6e0] flex-1">Duels aeris</span>
            <div className="flex items-center gap-2">
      ...
      <StatSection title="Pilota aturada i disciplina" icon={Flag} iconColor="#0ea5e9">
        ...
        <div className="border-t border-white/5 pt-2 mt-2">
          ...
          <div className="flex items-center justify-between py-2">
            <span className="text-xs text-[#d0d6e0] flex-1">Penals marcats / aturats</span>
            <div className="flex items-center gap-2">
      ...
      <StatSection title="Porteria" icon={GloveIcon} iconColor="#14b8a6">
        ...
        <div className="border-t border-white/5 pt-2 mt-2">
          <p className="text-[9px] text-[#62666d] uppercase tracking-wider mb-2">Manual</p>
          <div className="flex items-center justify-between py-2">
            <span className="text-xs text-[#d0d6e0] flex-1">Parades totals</span>
            <StatStepper value={summary.saves} onChange={(v) => update({ saves: v })} />
          </div>
          <div className="flex items-center justify-between py-2">
            <span className="text-xs text-[#d0d6e0] flex-1">Sortides per alt</span>
      ...
      <div className="mb-5">
        <label className="text-xs text-[#8a8f98] mb-1 block">Capita del partit</label>
        <select
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          <label className="text-xs text-[#8a8f98] mb-2 block flex items-center gap-1">
            <Cloud size={11} /> Temps
          </label>
      ...
      {/* ─── Overall rating ─── */}
      <div className="mb-6">
        <label className="text-xs text-[#8a8f98] mb-2 block">Valoracio global del partit</label>
        <RatingSlider
      ...
      {/* ─── Phase ratings ─── */}
      <div className="mb-6">
        <label className="text-xs text-[#8a8f98] mb-3 block">Valoracio per fases de joc (1-5)</label>
        <div className="space-y-2">
          {PHASE_KEYS.map((key) => (
            <div key={key} className="flex items-center gap-3">
              <span className="text-xs text-[#d0d6e0] w-36">{PHASE_LABELS[key]}</span>
      ...
      {/* ─── Tactical approach ─── */}
      <div className="mb-6">
        <label className="text-xs text-[#8a8f98] mb-2 block">Enfocament tactic</label>
            ...
            <button
              ...
              className={`flex items-center gap-2 py-2.5 px-3 rounded-xl text-sm font-medium transition-all ${
                summary.tactical_approach === value
                  ...
                  : 'bg-white/3 text-[#8a8f98] border border-white/8 hover:bg-white/5'
              }`}
            >
              <span>{emoji}</span>
      ...
      {/* ─── Text notes ─── */}
      <div className="mb-5">
        <label className="text-xs text-[#8a8f98] mb-1.5 block">Moments clau</label>
        <textarea
          ...
          rows={3}
          className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder-[#62666d] focus:outline-none focus:border-green-500/50 resize-none"
        />
          ...
          <div className="mt-2 flex flex-wrap gap-1">
            {state.events.filter(e => ['goal', 'red_card', 'injury'].includes(e.event_type)).slice(0, 5).map((e, i) => (
              <button
                ...
                className="text-[10px] px-2 py-0.5 bg-white/5 text-[#8a8f98] rounded hover:text-white transition-colors"
              >
      ...
      <div className="mb-5">
        <label className="text-xs text-[#8a8f98] mb-1.5 block">Analisi del rival</label>
        <textarea
      ...
      <div className="mb-5">
        <label className="text-xs text-[#8a8f98] mb-1.5 block">Arees de millora</label>
        <textarea
      ...
      <div className="mb-8">
        <label className="text-xs text-[#8a8f98] mb-1.5 block">Focus d&apos;entrenament per la setmana vinent</label>
        <textarea
      ...
      <div className="flex gap-3">
        <button
          ...
          disabled={saving}
          className="flex-1 flex items-center justify-center gap-2 py-3 bg-white/5 border border-white/10 text-[#8a8f98] text-sm rounded-xl hover:bg-white/10 transition-all disabled:opacity-30"
        >
          <Save size={14} />
          Desar esborrany
        </button>
        <button
          ...
          disabled={saving}
          className="flex-[2] flex items-center justify-center gap-2 py-3 bg-gradient-to-r bg-[#22c55e] hover:from-green-500 hover:to-[#22c55e] text-white text-sm font-semibold rounded-xl transition-all disabled:opacity-30"
        >
          {saving ? (
            <>
